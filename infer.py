...
class Osage_SAMD:
    def __init__(self, model_path):
        ...
        self.netG = Generator(config.latent_size, config.channels, upsample_first=False, bn_type='batch')
        self.netE = Encoder(config.image_size, config.latent_size, bn_type='instance')
        self.netD = Discriminator(config.image_size, config.latent_size, bn_type='layer')
        
        self.netG.load_state_dict(checkpoint['generator'])
        self.netE.load_state_dict(checkpoint['encoder'])
        self.netD.load_state_dict(checkpoint['discriminator'])
        
        self.sabv = SABV(FIS_ENABLED=True, N=5, sample=0.05, FIS_THREADING_ENABLED=True)
